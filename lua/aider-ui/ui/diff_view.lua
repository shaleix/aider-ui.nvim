...
local function handle_render_file(bufnr, file, start_lnum, end_lnum)
...
---@return integer Next starting line number
local function render_file(bufnr, file, start_lnum, end_lnum, winid)
  if file.opened then
          ...
          timer:start(
            ...
            vim.schedule_wrap(function()
              ...
              if elapsed >= max_wait_time or (current_length > 0 and current_length == last_length) then
                ...
                handle_render_file(bufnr, file, start_lnum, end_lnum)
                return
              end
  ...
  return handle_render_file(bufnr, file, start_lnum, end_lnum)
end
...
local function init_render_diff(popup, diff_files)
  local lines = {}
  ...
  local function toggle_file_fold(current_line)
    ...
    if target_item then
      ...
      render_file(popup.bufnr, target_item, line_index, end_lnum, popup.winid)
  ...
  for _, item in ipairs(diff_files) do
    ...
    current_lnum = render_file(bufnr, item, current_lnum, end_lnum, popup.winid)
  end
